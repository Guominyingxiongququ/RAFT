...
@PIPELINES.register_module()
class GenerateDNTestFrameIndices:
    """Generate frame index for CRVD datasets. It also performs
    temporal augmention with random interval.

    test from frame one to frame seven with sequence padding
...
@PIPELINES.register_module()
class GenerateRAWDNTestFrameIndices:
    """Generate frame index for CRVD datasets. It also performs
    temporal augmention with random interval.
    ...
    def __call__(self, results):
        ...
        for v in neighbor_list:
            if v<1:
                ...
                v = 7
            lq_frame = osp.join(lq_path_root, clip_name, iso_name,
                                f'frame{v}_noisy{noisy_idx}.tiff')
            lq_path.append(lq_frame)
...
class GenerateDNFrameIndices:
    ...
    def __call__(self, results):
        ...
        Returns:
            dict: A dict containing the processed data and information.
        """
        results_split = results['key'].split('/')
        if len(results_split)==3:
            clip_name, iso_name, frame_name = results_split
            # key example: scene2/ISO3200/frame2_clean_and_slightly_denoised
            frame_idx_str = frame_name.split("_")[0][5:]
            center_frame_idx = int(frame_idx_str)
            num_half_frames = results['num_input_frames'] // 2
            ...
            # MOT key example: "MOT17-11_raw/000900_raw.png (1920, 1080, 3)"
            clip_name, frame_name = results['key'].split(
            '/')
            frame_idx_str = frame_name.split("_")[0]
            center_frame_idx = int(frame_idx_str)
            num_half_frames = results['num_input_frames'] // 2
            max_frame_num = results.get('max_frame_num', self.frames_per_clip)
            frames_per_clip = min(self.frames_per_clip, max_frame_num)
            ...
            iso = np.random.choice(iso_list)
            for v in neighbor_list:
                rand_int = np.random.randint(0, 2)
                lq_frame = osp.join(lq_path_root, clip_name, 
                f'{v:06}_raw_iso{iso}_noisy{rand_int}.png')
                lq_path.append(lq_frame)

            gt_path = [osp.join(gt_path_root, results['key'] + '.png')]
        results['lq_path'] = lq_path
        results['gt_path'] = gt_path
        results['interval'] = interval
...
@PIPELINES.register_module()
class GenerateRAWDNFrameIndices:
    """Generate frame index for CRVD datasets. It also performs
    temporal augmention with random interval.
    ...
    def __call__(self, results):
        ...
        for v in neighbor_list:
            rand_int = np.random.randint(0, 10)
            lq_frame = osp.join(lq_path_root, clip_name, iso_name,
                                f'frame{v}_noisy{rand_int}.tiff')
            lq_path.append(lq_frame)
...
@PIPELINES.register_module()
class GeneratePreDNFrameIndices:
    """Generate frame index for CRVD datasets. It also performs
    temporal augmention with random interval.

    Required keys: lq_path, dn_path, gt_path, key, num_input_frames
    Added or modified keys:  lq_path, gt_path, interval, reverse
    ...
    def __call__(self, results):
        ...
        for v in neighbor_list:
            ...
            lq_frame = osp.join(lq_path_root, clip_name, iso_name,
                                f'frame{v}_noisy{rand_int}.png')
            dn_frame = osp.join(dn_path_root, clip_name, iso_name,
                                f'frame{v}_noisy{rand_int}.png')
            lq_path.append(lq_frame)
            dn_path.append(dn_frame)
        
        gt_frame_name = f'frame{center_frame_idx}_clean_and_slightly_denoised'
        gt_path = [osp.join(gt_path_root, clip_name, iso_name,
                   f'{gt_frame_name}.png')]
        results['lq_path'] = lq_path
        results['dn_path'] = dn_path
        results['gt_path'] = gt_path
        results['interval'] = interval
